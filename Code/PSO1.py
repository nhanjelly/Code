...
coords = np.array([[x, y] for _, x, y, _, _, _, _ in data], dtype=float)
demand = np.array([d for _, _, _, d, _, _, _ in data], dtype=int)
ready_time = np.array([r for _, _, _, _, r, _, _ in data], dtype=int)
due_time = np.array([d for _, _, _, _, _, d, _ in data], dtype=int)
service_time = np.array([s for _, _, _, _, _, _, s in data], dtype=int)

NUM_CUSTOMERS = len(data) - 1
...
def cost_function(route):
    ...
    for customer in route:
        ...
        if arrival < ready_time[customer]:
            arrival = ready_time[customer]

        if arrival > due_time[customer]:
            total_distance += 1000
            arrival = due_time[customer]

        time = arrival + service_time[customer]
...
class Particle:
    def __init__(self):
        self.position = random.sample(customers, len(customers))
        self.velocity = []
...
def PSO():
    ...
    gbest = min(swarm, key=lambda p: p.best_fitness)
    gbest_position = gbest.best_position[:]
    gbest_fitness = gbest.best_fitness

    for it in range(MAX_ITER):
        for p in swarm:
            swap_pbest = get_swap_sequence(p.position, p.best_position)
            swap_gbest = get_swap_sequence(p.position, gbest_position)

            new_velocity = []

            for s in p.velocity:
                if random.random() < w:
                    new_velocity.append(s)
            ...
            p.position = apply_swaps(p.position, p.velocity)

            fitness = cost_function(p.position)
        ...
        print(f"Iter {it+1}, Best = {gbest_fitness:.2f}")

    return gbest_position, gbest_fitness

if __name__ == "__main__":
    print("Số khách hàng:", NUM_CUSTOMERS)
    print("Sức chứa xe:", VEHICLE_CAPACITY)

    best_route, best_cost = PSO()
